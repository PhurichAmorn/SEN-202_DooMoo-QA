...
void main() {
  ...
  group('Load Test · PigMath.pixelToMm', () {
    test('1,000 calls at normal camera params → throughput & avg latency', () {
      ...
      print('══════════════════════════════════════════════════');
      print(' Total time   : ${totalUs} µs  (${(totalUs / 1000).toStringAsFixed(2)} ms)');
    ...
    test('10,000 calls with varied pixel lengths → stability under sustained load', () {
      const n = 10000;
      final sw = Stopwatch()..start();
      int nullResults = 0;
  ...
  group('Load Test · PigMath.estimateWeight', () {
    test('1,000 calls with realistic pig dimensions → throughput & avg latency', () {
      const n = 1000;
      final sw = Stopwatch()..start();
    ...
    test('10,000 calls with full value range → measures error rate', () {
      ...
      print(' Avg per call : ${avgUs.toStringAsFixed(3)} µs');
      print(' Throughput   : ${throughput.toStringAsFixed(0)} calls / sec');
      print(' "-" (invalid): $dashCount / $n  (${errorRate.toStringAsFixed(1)} %)');
      print('══════════════════════════════════════════════════\n');
  ...
  group('Stress Test · Full Measurement Pipeline', () {
    test('100,000 full pipeline cycles → finds throughput ceiling', () {
      ...
      print('══════════════════════════════════════════════════');
      print(' Total time    : ${totalMs} ms  (${(totalMs / 1000).toStringAsFixed(2)} s)');
      print(' Avg per cycle : ${avgUs.toStringAsFixed(3)} µs');
      print(' Throughput    : ${throughput.toStringAsFixed(0)} cycles / sec');
      print(' Success rate  : $successCount / $n  (${successRate.toStringAsFixed(1)} %)');
      print('══════════════════════════════════════════════════\n');

      expect(avgUs, lessThan(2000),
          reason: 'Full pipeline (4× pixelToMm + estimateWeight) must stay under 2 ms per cycle');
      expect(successRate, greaterThan(95.0),
  ...
  group('Stress Test · Input Validation Under Load', () {
    test('50,000 mixed valid/invalid calls → correct error handling at speed', () {
      const n = 50000;
      final sw = Stopwatch()..start();
      int nullCount = 0;
      int validCount = 0;
  ...
  group('Scalability Test · PigMeasurements.fromMask (PCA)', () {
    ...
    for (final size in maskSizes) {
      test('$size × $size mask → avg PCA time, edge-pixel count, measurements', () {
        ...
        // Warm-up
        PigMeasurements.fromMask(mask, bbox, imageWidth: size, imageHeight: size);
        ...
        print(' Avg time     : ${avgMs.toStringAsFixed(2)} ms  ${_bar(fraction)}');
        if (result != null) {
          print(' Body length  : ${result.length.toStringAsFixed(1)} px');
          print(' Chest width  : ${result.widthTop.toStringAsFixed(1)} px');
          print(' Abdom. width : ${result.widthMiddle.toStringAsFixed(1)} px');
          print(' Hip width    : ${result.widthBottom.toStringAsFixed(1)} px');
        } else {
        ...
        expect(result, isNotNull,
            reason: 'A clear elliptical mask must always produce a valid measurement');
    ...
    test('Scalability summary — prints how time grows with mask area', () {
      ...
      if (t100 > 0) {
        ...
        expect(growthFactor, lessThan(100),
